...
interface Metadata { title: string; sourceUrl: string; publisher: string; revisedOn: string; }
...
export default function AdminDocumentsPage() {
  ...
  const [metadata, setMetadata] = useState<Metadata>({ title: '', sourceUrl: '', publisher: '', revisedOn: '' });
  ...
  async function submit(register: boolean) {
    ...
    try {
      const fields = { ...metadata, reviewedHash: register ? preview?.hash ?? '' : '', expectedVersion: preview?.currentVersion ?? '', replaceApproved };
