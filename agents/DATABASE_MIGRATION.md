...
이 문서는 **원래 파일 클러스터의 CRC 오류 당시 기록**이다. 이후 사용자 요청으로 빈 DB용 Flyway 구현·격리 검증을 진행했다. 최신 결과·실행·복구·실제 적용 검수안은 [Flyway 작업서](FLYWAY_RUNBOOK.md)를 읽는다. 아래 미구현·중단 표시는 당시 상태이며 원래 D: DB의 복구 성공을 주장하지 않는다.
