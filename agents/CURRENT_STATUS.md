...
- 공식 카카오 우편번호 SDK의 팝업 주소 검색과 상세주소 입력을 추가했다. 기존 address 필드로 합쳐 보내며 DB 스키마는 변경하지 않는다. 검색 서비스 로딩 실패 시 안내하고 직접 주소 입력도 유지한다.
...
- 앱 내 브라우저에서는 주소 검색 창 연결이 완료되지 않았고, 날짜 입력 자동 검사 중 테스트 탭이 종료되었다. 주소 선택→입력 반영, 비밀번호 불일치 안내 및 정상 다음 단계 이동은 일반 Chrome에서 추가 확인해야 한다. 브라우저 자동 검증 통과로 간주하지 않는다.
